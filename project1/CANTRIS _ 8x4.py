...
class AI():
    def __init__(self):
        ...
        self.children=[] #record next possible move
        self.val=None #root pos does not matter,so set -1,-1
        self.pos=-1,-1 #record value after alpha-beta pruning
    ...
    def make_decision_MCTS(self):
        ...
        time_end=time.time()
        #select the move that can win the game
        for i in range(col):
            s=copy.deepcopy(self)
            pts = s.make_move(row-1,i)
            s.mypoints += pts
            if s.gameover and (s.mypoints-s.oppopoints>0):
                return row-1,i
        #let s0=current state ,and run MCTS process
        root=Node()
        count=0 #count simulation times
        time_limit=5
    ...
    def alphabeta(self,depth, a, b, maximizingPlayer):
        ...
        if maximizingPlayer:
            for i in self.children:
                a=max(a,i.alphabeta(depth-1,a,b,False))
                i.val=a
                if b<=a:
                    break #b cut off
            return a
            ...
            for i in self.children:
                b=min(b,i.alphabeta(depth-1,a,b,True))
                i.val=b
                if b<=a:
                    break #a cut off
            return b
    ...
    def make_decision(self):
        ...
        #own side
        if (s.step%2) == s.turn:
            val=s.alphabeta(depth,float('-inf'),float('inf'),True)
        #opponent side
        else:
            val=s.alphabeta(depth,float('-inf'),float('inf'),False)
